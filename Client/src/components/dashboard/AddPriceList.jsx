...
const AddPriceList = () => {
    ...
    useEffect(() => {
        const fetchAdd = async () => {
            try {
                const addPrice = await pricelist.CreateAPriceForAlbums(auth.accessToken, photo_album_id, name, price,
                ...
                if (addPrice.statusCode === 201) {
                    notify(addPrice.response.message, 'success');
                    setLoading(false);
                    setSubmit(false);
                } else {
                    notify(addPrice.error.message);
                    setLoading(false);
                    setSubmit(false);
                }
            } catch (error) {
