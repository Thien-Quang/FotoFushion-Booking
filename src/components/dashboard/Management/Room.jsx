...
const Room = () => {
  ...
  const handleDeleteImage = async () => {
    try {
      ...
      if (deleteSuccess) {
        const deletePhoto = await photo.deletePhotoByRoomId(
          auth.accessToken,
          room_id
        );
        if (deletePhoto.statusCode === 204) {
          const deleteCostumer = await roomApi.deleteRoom(
            auth.accessToken,
            room_id
          );
          ...
          notify("Xóa hình ảnh phòng chụp hình không thành công", "error");
        }
      } else {
    ...
    <div>
      ...
      <div className="w-full flex items-center justify-center m-2">
        <span className="text-2xl font-semibold">
          Quản Lí Phòng Concept Chụp Hình
        </span>
      </div>
              ...
              {filteredStudios.map((item) => {
                return (
                  <Table.Row className="bg-white dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <Table.Cell>
                      {item.is_status ? "Đang sử dụng" : "Chưa sử dụng"}
                    </Table.Cell>
