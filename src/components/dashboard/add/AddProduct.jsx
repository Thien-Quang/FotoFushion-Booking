...
const AddProduct = () => {
  ...
  useEffect(() => {
    const fetchAdd = async () => {
      try {
        const addProduct = await product.addAProduct(
          auth.accessToken,
          name,
          description,
          category,
    ...
    if (prod_id && imageUrls.length === imageUploads.length) {
      ...
      const performAddProduct = async () => {
        try {
          ...
          await Promise.all(
            imageUrls.map(async (url_photo) => {
              const addProduct = await photo.updateProductPhoto(
                auth.accessToken,
                img_name,
                url_photo,
                prod_id
            ...
            <form action="" onSubmit={(e) => handleSubmit(e)}>
              ...
              <div className="flex flex-col mb-6">
                <label
                  ...
                  htmlFor=""
                >
                  Nhập loại sản phẩm (vd Tranh treo tường , Poster idol , Poster
                  cầu thủ , film , albums...s)
                </label>
                <input
                  id="categoryInput"
                  className="px-4 py-3 border-2 border-[#afafaf] rounded-lg shadow-lg outline-none focus:border-primaryColor placeholder:text-lg text-lg"
                  ...
                  onChange={(event) => {
                    setimg_name(event.target.value);
